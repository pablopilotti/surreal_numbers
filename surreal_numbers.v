...
Fixpoint left (s : symbol) : list symbol :=
match s with
| pair l _ => l
end.

Fixpoint right (s : symbol) : list symbol :=
match s with
| pair _ r => r
end.
Hint Unfold left right.

(* Ejemplo de Simbolos *)
Definition n0 := ([],[]).
Definition n_1 := ([],[n0]).
Definition n1 := ([n0],[]).
Hint Unfold n0 n1 n_1.
...
Hint Resolve forall_ngeq_l forall_ngeq_r.
...
Hint Resolve leq_n ngeq_n.

Check ngeq_n.

Definition is_number (x: symbol) := ngeq (left x) (right x).
Hint Unfold is_number.

(*Pruebas de que 0, 1 y -1 son numeros y 0 <= 0, *)
Lemma Zero_is_number: (is_number n0). Proof. unfold is_number. unfold n0. unfold left. apply forall_ngeq_l. intros. tauto. Qed.
...
Proof.
intros.
elim (classic ((ngeq (left X) [Y] /\ ngeq [X] (right Y)))).
intros.
apply leq_def in H0.
tauto.
tauto.
...
destruct H.
destruct H0.
apply not_forall_ngeq_l in H1.
apply not_forall_exists_not_l in H1.
elim H1.
intros.
exists x.
intros.
...
destruct H.
destruct H0.
apply not_forall_ngeq_r in H1.
apply not_forall_exists_not_r in H1.
elim H1.
intros z.
intros.
exists z.
